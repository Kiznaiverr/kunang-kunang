...
function getLoopStatus(repeatMode: QueueRepeatMode): string {
  switch (repeatMode) {
...
const queueCommand: Command = {
  ...
  execute: async (message: Message, args: string[], bot: any) => {
    ...
    Logger.debug(
      `Queue command initiated by ${message.author.username} in guild ${message.guild?.name}`,
      "QueueCommand"
    );
    ...
    Logger.debug(
      `Queue has ${tracks.length + 1} total tracks (1 playing + ${
        tracks.length
      } queued)`,
      "QueueCommand"
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: `[**${currentTrack.title}**](${
          currentTrack.url || "https://example.com"
        })\n\`${currentTrack.author || "Unknown"}\` • \`${
          currentTrack.duration || "Unknown"
        }\``,
        inline: false,
      })
      .setTimestamp()
      .setFooter({
        text: `Loop: ${getLoopStatus(queue.repeatMode)} • ${
          tracks.length + 1
        } songs • Total duration: ${calculateTotalDuration(
          currentTrack,
          tracks
        )}`,
      });
