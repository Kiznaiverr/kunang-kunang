...
class MusicOverlay {
  ...
  updatePreset(presetNumber) {
    ...
    if (this.elements.overlayContainer) {
      this.elements.overlayContainer.classList.remove(
        "preset-1",
        "preset-2",
        "preset-3",
        "preset-4"
  ...
  updateQueue(queue) {
    ...
    if (!queue || queue.length === 0) {
      queueList.innerHTML =
        '<span class="queue-item-inline">No songs in queue</span>';
      return;
    }
  ...
  updateLoop(loopMode) {
    if (this.elements.loopStatus) {
      this.elements.loopStatus.textContent = `Loop: ${loopMode || "Off"}`;
    }
  }
